...
const WA_TEMPLATES = [
  ...
  },
  {
    ...
    getMessage: (name: string, pos: string) => `Halo *${name}*,\n\nTerima kasih telah melamar di RAPID TECHNOLOGY. Untuk saat ini kualifikasi Anda belum sesuai dengan kebutuhan kami untuk posisi *${pos}*.\n\nData Anda akan kami simpan untuk kebutuhan lowongan di masa mendatang. Sukses selalu!`
  },
  {
    id: 'custom',
    label: '💬 Chat Manual',
...
export const Dashboard: React.FC<DashboardProps> = ({ onLogout }) => {
  ...
  const fetchDashboardData = useCallback(async () => {
      ...
      const eduData = [
          { label: 'SMA/SMK', value: eduCounts['SMA/SMK'] || 0, color: '#ef4444' },
          { label: 'D3', value: eduCounts['D3'] || 0, color: '#f97316' },
          { label: 'S1', value: eduCounts['S1'] || 0, color: '#eab308' },
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex font-sans">
      <aside className={`bg-white text-slate-600 flex flex-col fixed h-full z-20 border-r border-slate-200 transition-all duration-300 ${isSidebarCollapsed ? 'w-20' : 'w-64'}`}>
        <div className="h-16 flex items-center justify-between px-4 border-b border-slate-100">
          {!isSidebarCollapsed ? (
            <img src="https://i.imgur.com/ASLIsER.png" alt="Logo" className="w-28 h-auto object-contain ml-2" />
          ) : (
            <img src="https://i.imgur.com/DWDHKH2.png" alt="Logo" className="w-8 h-8 object-contain mx-auto" />
          )}
          <button onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)} className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-400 hover:text-slate-600 transition-colors">
            {isSidebarCollapsed ? <ChevronRight size={20} /> : <ChevronLeft size={20} />}
          </button>
        </div>
        <nav className="flex-1 py-6 px-3 space-y-1 overflow-y-auto overflow-x-hidden">
          <button onClick={() => setActiveTab('dashboard')} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all ${activeTab === 'dashboard' ? 'bg-brand-50 text-brand-600 font-medium' : 'hover:bg-slate-50 text-slate-600'} ${isSidebarCollapsed ? 'justify-center' : ''}`} title="Dashboard">
            <LayoutDashboard size={18} className={activeTab === 'dashboard' ? 'text-brand-600' : 'text-slate-400'} /> 
            {!isSidebarCollapsed && <span>Dashboard</span>}
          </button>

          <button onClick={() => setActiveTab('monitoring')} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all ${activeTab === 'monitoring' ? 'bg-brand-50 text-brand-600 font-medium' : 'hover:bg-slate-50 text-slate-600'} ${isSidebarCollapsed ? 'justify-center' : ''}`} title="Monitoring Klien">
            <LayoutGrid size={18} className={activeTab === 'monitoring' ? 'text-brand-600' : 'text-slate-400'} /> 
            {!isSidebarCollapsed && <span>Monitoring Klien</span>}
          </button>

          {!isSidebarCollapsed ? (
            <div className="pt-4 pb-2 px-3 text-xs font-semibold text-slate-400 uppercase tracking-wider">Pipeline</div>
          ) : (
            <div className="pt-4 pb-2 text-center border-t border-slate-100 mt-2"></div>
          )}
          ...
          {['talent_pool', 'process', 'interview', 'hired', 'rejected'].map((tab) => {
             ...
             return (
             <button key={tab} onClick={() => setActiveTab(tab as any)} className={`w-full flex items-center px-3 py-2.5 rounded-lg transition-all ${activeTab === tab ? 'bg-brand-50 text-brand-600 font-medium' : 'hover:bg-slate-50 text-slate-600'} ${isSidebarCollapsed ? 'justify-center' : 'justify-between'}`} title={tab.replace('_', ' ')}>
                <div className="flex items-center gap-3 capitalize">
                    <Icon size={18} className={activeTab === tab ? 'text-brand-600' : 'text-slate-400'} />
                    {!isSidebarCollapsed && <span>{tab.replace('_', ' ')}</span>}
                </div>
                {/* @ts-ignore */}
                {!isSidebarCollapsed && <span className={`text-xs px-2 py-0.5 rounded-full min-w-[24px] text-center ${activeTab === tab ? 'bg-brand-100 text-brand-700' : 'bg-slate-100 text-slate-500'}`}>{stats[tab === 'talent_pool' ? 'new' : tab]}</span>}
             </button>
             );
          })}
          ...
          <button onClick={() => setActiveTab('master_data')} className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all ${activeTab === 'master_data' ? 'bg-brand-50 text-brand-600 font-medium' : 'hover:bg-slate-50 text-slate-600'} ${isSidebarCollapsed ? 'justify-center' : ''}`} title="Master Data">
            <Settings size={18} className={activeTab === 'master_data' ? 'text-brand-600' : 'text-slate-400'} /> 
            {!isSidebarCollapsed && <span>Master Data</span>}
          </button>
        </nav>
        <div className="p-4 border-t border-slate-100">
            <button onClick={onLogout} className={`w-full flex items-center gap-2 text-red-500 py-2.5 hover:bg-red-50 rounded-lg transition-colors ${isSidebarCollapsed ? 'justify-center' : 'justify-center'}`} title="Keluar">
                <LogOut size={18} /> 
        ...
        {activeTab === 'dashboard' ? (
            ...
            <div className="space-y-6 animate-fadeIn">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                    <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-100 relative overflow-hidden">
                        <div className="absolute right-0 top-0 opacity-5 text-brand-500 transform translate-x-4 -translate-y-4"><User size={100}/></div>
                        <div className="text-slate-500 text-sm font-medium mb-1">Total Pelamar</div>
                        <div className="text-4xl font-bold text-slate-800">{stats.total}</div>
                        <div className="mt-4 text-xs bg-brand-50 text-brand-600 w-fit px-2 py-1 rounded-md font-medium">Semua Waktu</div>
                    </div>
                    <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-100 relative overflow-hidden">
                        <div className="absolute right-0 top-0 opacity-5 text-indigo-500 transform translate-x-4 -translate-y-4"><FileText size={100}/></div>
                        <div className="text-slate-500 text-sm font-medium mb-1">Baru Masuk</div>
                        <div className="text-4xl font-bold text-slate-800">{stats.new}</div>
                        <div className="mt-4 text-xs bg-indigo-50 text-indigo-600 w-fit px-2 py-1 rounded-md font-medium">Perlu Diproses</div>
                    </div>
                    <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-100 relative overflow-hidden">
                        <div className="absolute right-0 top-0 opacity-5 text-amber-500 transform translate-x-4 -translate-y-4"><Loader2 size={100}/></div>
                        <div className="text-slate-500 text-sm font-medium mb-1">Sedang Proses</div>
                        <div className="text-4xl font-bold text-slate-800">{stats.process}</div>
                        <div className="mt-4 text-xs bg-amber-50 text-amber-600 w-fit px-2 py-1 rounded-md font-medium">Interview & Psikotes</div>
                    </div>
                    <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-100 relative overflow-hidden">
                        <div className="absolute right-0 top-0 opacity-5 text-emerald-500 transform translate-x-4 -translate-y-4"><CheckCircle size={100}/></div>
                        <div className="text-slate-500 text-sm font-medium mb-1">Diterima</div>
                        <div className="text-4xl font-bold text-slate-800">{stats.hired}</div>
                        <div className="mt-4 text-xs bg-emerald-50 text-emerald-600 w-fit px-2 py-1 rounded-md font-medium">Karyawan Baru</div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-2 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
                        ...
                        <SimpleLineChart data={dashboardMetrics.trend} color="#ef4444" />
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden animate-fadeIn">
              <div className="p-4 border-b border-slate-200 flex flex-col md:flex-row gap-4 justify-between items-center bg-slate-50">
                ...
                <button className="flex items-center gap-2 bg-brand-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-brand-700 transition-colors" onClick={() => alert("Fitur Tambah Kandidat Manual akan segera hadir.")}>
              ...
              <div className="overflow-x-auto min-h-[400px]">
                {loading ? (
                    ...
                    <thead className="bg-brand-600 text-white text-xs uppercase font-semibold">
                        <tr>
                        <th className="p-4 w-10"><button onClick={toggleSelectAll}><CheckSquare size={16} className={selectedIds.length > 0 ? "text-white" : "text-brand-200"} /></button></th>
