...
export const JobForm: React.FC<JobFormProps> = ({ onBack }) => {
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.hasPengalamanKerja) {
        ...
        if (!formData.noHpAtasan) errors.noHpAtasan = "No HP atasan wajib diisi.";
    }
    ...
    <div className="min-h-screen bg-gray-50 pb-12 animate-fadeIn">
      ...
      <div className="h-72 bg-slate-900 relative overflow-hidden group">
         ...
         <div className="absolute inset-0 flex flex-col items-center justify-center text-white px-4 pt-10">
           ...
           <p className="text-lg text-slate-300 max-w-xl text-center font-light">Lengkapi data diri Anda untuk bergabung dengan tim terbaik RAPID TECHNOLOGY.</p>
             ...
             {formData.hasPengalamanKerja && (
               <div className="bg-slate-50 border border-slate-200 p-6 rounded-xl space-y-4 animate-fadeIn">
                   ...
                   <div className="pt-4 mt-2 border-t border-slate-200">
                       <h5 className="font-bold text-sm text-slate-500 mb-3 uppercase tracking-wider flex items-center gap-2"><User size={14}/> Kontak Referensi (Atasan)</h5>
                       <div className="grid md:grid-cols-3 gap-4">
                           <InputField label="Nama Atasan" name="namaAtasan" value={formData.namaAtasan} onChange={handleChange} required error={validationErrors.namaAtasan} />
                           <InputField label="Jabatan Atasan" name="jabatanAtasan" value={formData.jabatanAtasan} onChange={handleChange} required error={validationErrors.jabatanAtasan} />
                           <InputField label="No HP Atasan" name="noHpAtasan" value={formData.noHpAtasan} onChange={handleChange} required error={validationErrors.noHpAtasan} type="tel" placeholder="08..." />
                       </div>
                   </div>
               </div>
             )}
             ...
             <div className="flex gap-5 relative z-10 flex-col md:flex-row">
                ...
                <div className="flex-1">
                   ...
                   <p className="text-sm text-gray-500 mb-5 leading-relaxed">
                      Dengan mengirimkan formulir ini, Anda menyatakan bahwa seluruh data yang diisi adalah benar dan dapat dipertanggungjawabkan. Data Anda dilindungi enkripsi dan hanya digunakan untuk kepentingan rekrutmen RAPID TECHNOLOGY.
                   </p>
